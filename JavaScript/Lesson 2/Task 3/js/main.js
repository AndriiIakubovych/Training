...
document.write("<br/>Массив, полученный из элементов матрицы, лежащих по периметру треугольника, образованного двумя сторонами матрицы и главной диагональю по часовой стрелке:<br/>");
for (var i = 0; i < (SIZE - 1) * 3; i++)
{
    if (i <= SIZE - 1)
        array[i] = matrix[i][i];
    if (i > SIZE - 1 && i <= (SIZE - 1) * 2)
        array[i] = matrix[SIZE - 1][(SIZE - 1) * 2 - i];
    if (i > (SIZE - 1) * 2)
        array[i] = matrix[(SIZE - 1) * 3 - i][0];
