...
document.write("<br/>Массив, полученный из элементов матрицы, лежащих по периметру против часовой стрелки:<br/>");
for (var i = 0; i < (SIZE - 1) * 4; i++)
{
    if (i <= SIZE - 1)
        array[i] = matrix[i][0];
    if (i > SIZE - 1 && i <= (SIZE - 1) * 2)
        array[i] = matrix[SIZE - 1][i - (SIZE - 1)];
    if (i > (SIZE - 1) * 2 && i <= (SIZE - 1) * 3)
        array[i] = matrix[(SIZE - 1) * 3 - i][SIZE - 1];
    if (i > (SIZE - 1) * 3)
        array[i] = matrix[0][(SIZE - 1) * 4 - i];
